...
action :install do
  execute "install NPM package #{new_resource.name}" do
    ...
    not_if { npm_package_installed?(new_resource.package, new_resource.version, new_resource.path) }
  end
end
...
action :uninstall do
  execute "uninstall NPM package #{new_resource.package}" do
    ...
    group new_resource.group
    environment 'HOME' => ::Dir.home(new_resource.user), 'USER' => new_resource.user if new_resource.user
    only_if { npm_package_installed?(new_resource.package, new_resource.version, new_resource.path) }
  end
end
